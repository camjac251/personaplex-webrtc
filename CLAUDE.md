...
- `uv run python moshi/tests/test_model_env.py` - checkpoint pins, strict loading, and offload-device tests.
...
- `uv run python moshi/tests/test_qualification.py` - privacy-safe duplex comparison schema and run-identity tests.
- `uv run python moshi/tests/test_runtime_metrics.py` - bounded realtime runtime-metrics tests.
- `uv run python moshi/tests/test_voice_analysis.py` - content-free voice-reference analysis tests.
- `uv run python moshi/tests/test_voice_enrollment.py` - enrollment identity, retention, and deletion contract tests.
- `uv run python moshi/tests/test_voice_state_causality.py` - voice-state causality experiment contract tests.
...
- Auto-rewind: `_pad_force_remaining` tripping `COLLAPSE_TRIGGER_THRESHOLD` times in `COLLAPSE_WINDOW_SEC` restores the latest snapshot via `set_streaming_state_inplace` and resets sampling/anti-collapse controls to the active model's safe defaults. Manual/bookmark rewind preserves tuning. Restore preflights LM, Mimi, and CPU/CUDA RNG as one logical point before applying either module; always pass `dict(state_dict)` because apply consumes keys. Baselines and named bookmarks are pageable-CPU snapshots admitted against a 24 GiB host budget and 4 GiB available-memory floor by default. Periodic recovery, when explicitly enabled, retains only the newest GPU snapshot under a 6 GiB budget and 2 GiB VRAM floor. Capture failure is nonfatal and cannot publish a partial bookmark. The measured Caption-CFG state is 3,162,118,280 bytes (2.945 GiB), not 1.6 GB. Auto-rewind accepts snapshots younger than `AUTO_REWIND_SNAPSHOT_MAX_AGE_SEC` (90 s), so with periodic snapshots off it goes inert after the baseline ages out; a trip that finds no usable snapshot still emits a warn `collapse` event (`triggers`, `window_sec`, `reason` `no_snapshot`/`stale_snapshot`, `snapshot_age_sec`) so the detection is visible even when recovery is unavailable. Snapshot capture defers while a context drip is active.
...
- Depformer early exit (`--depformer-early-exit` / `PERSONAPLEX_DEPFORMER_EARLY_EXIT`) is an experiment and defaults off. Values 8-16 skip trailing codebooks whose user-stream values are already supplied while consuming the stock sampler's RNG progression. It must stay off until the declared GPU and quality gates confirm graph/runtime parity.
...
- Native-duplex defaults: `audio_temperature=0.8`, `padding_bonus=0.0`, `turn_onset_bias=0.0`, `repetition_penalty=1.0`, `max_turn_text_tokens=120`. `padding_bonus` applies only mid-turn (`_non_pad_streak > 0`, with a forced-text hold so a caption drip never re-arms it at reply onset); onset pacing belongs to `turn_onset_bias`, a live-tunable EPAD logit bias clamped to [-4, 4] that auto-rewind resets to 0. User configuration cannot lower max-turn below 40, and only caps at or above `COLLAPSE_SIGNAL_MIN_TURN_TOKENS` (120) count toward auto-rewind; lower caps truncate without feeding collapse detection. Max-turn counts text across brief PAD gaps and resets only after `REPETITION_TURN_BREAK_FRAMES` natural PAD/EPAD frames. Native mode never turns client-detected overlap into an automatic interrupt; Assisted mode does. The repetition ring uses the same natural turn boundary. Turn-cap accounting reads the text token through an async pinned D2H copy gated by a CUDA event; the hot path never blocks on a per-frame `.item()`.
- Stop is latched: `_stop_response_latched` forces assistant PAD and zero PCM until the next valid user turn completes. A fixed one-second gate alone is insufficient because the model can resume the abandoned answer. `STOP_LATCH_MAX_HOLD_SEC` (12 s) force-releases a latch whose release detectors starve in sustained room noise. Every release emits one `stop_latch` event (`reason` `user_turn` or `hold_ceiling`, numeric `held_sec`) so the dashboard timeline shows which trigger freed the model. Rewind/new-session reset the latch; transport resume preserves it.
- The slow `stat` envelope exposes cumulative inbound queue and outbound-buffer pressure; runtime summaries add bounded numeric snapshot bytes, timing, failure/rejection counts, and residency inventory. Keep those fields numeric and explicitly allowlisted; never put PCM, paths, SDP/candidates, or free-form secrets in telemetry.
...
- The `stat` envelope carries `opus_encode_failures`, a process-wide count of dropped-frame encode failures from the mono Opus patch, plus the shadow counters `text_starved_frames`/`text_starved_episodes` (sustained natural PAD/EPAD text over non-silent output; observability only, never a control input).
