...
Periodic full-state snapshots are on by default: a fresh session keeps one recent GPU-resident rolling recovery point refreshed once a minute so long-session auto-rewind stays armed after the session-start baseline ages out, and every capture defers until the same quiet boundary the context-inject gate trusts, so its roughly 416 ms frame-loop stall never lands on audible output. Each fresh session also keeps one CPU-resident baseline for manual Rewind, and confirmed bookmarks are CPU-resident as well. Captures are admitted against measured tensor bytes, a retained-byte budget, and a free-memory floor; rejection is nonfatal and never replaces an existing recovery point. The measured two-row Caption-CFG state is 2.945 GiB, so bookmark storage is server-authoritative and the dashboard waits for a `stored` receipt. Set `PERSONAPLEX_PERIODIC_SNAPSHOTS=0` (or pass `--no-periodic-snapshots`) to return to baseline-only recovery.

## Profiles and diagnostics

The dashboard includes checkpoint-aware **Balanced**, **Upstream**, **Concise**, and **Expressive** profiles. Upstream pins the NVIDIA/Kyutai reference operating point (text 0.7 / top-k 25, audio 0.8 / top-k 250 on every level, no repetition penalty, bare persona) as an A/B baseline. Expressive keeps text at 0.7 (the text stream decides turn timing) and takes its colour from the tested VARF4 voice, a personality descriptor, and acoustic temperature 0.9; it selects Native duplex only for the aligned checkpoint, while Base and unknown checkpoints use Assisted overlap handling. The selected mode travels to the server as `turn_handling` in the session config, so the server enforces it: Native rejects any client-sourced barge-in, and Assisted arms a server-side sustained-overlap stop keyed to the shared user-speech floor rather than the browser's much higher level meter, with mid-session toggles applied live. The semantic codebook (Mimi level 0) has its own temperature slider, independent of the acoustic levels, defaulting to 0.7 where upstream samples 0.8. Raw sampling controls remain available under Advanced.
